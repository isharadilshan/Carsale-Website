  ...
  <section class="content">
    ...
    <div class="box">
      <div class="box-header with-border">
        <h3 class="box-title">Cars Inventory</h3>
        <a class="col-md-offset-4 float-right btn btn-success" href="{{ route('inventory.create') }}">Add New</a>
        ...
        <div class="box">
          <div class="box-body">
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tbody>
                @foreach($inventoryCars as $item)
                  <tr>
                      <td>{{$loop->index+1}}</td>
                      <td>{{$item->carModel()->get()->first()->title}}</td>
                      <td>{{$item->engine_no}}</td>
                      <td>{{$item->chasis_no}}</td>
                      <td><a href="{{ route('inventory.edit',$item->id) }}"><span class="glyphicon glyphicon-edit"></span></a></td>
                      <td>
                          <form method="post"id="delete-form-{{ $item->id }}" action="{{ route('inventory.destroy',$item->id) }}" style="dispaly: none">
                              {{ csrf_field()}}
                              {{ method_field('DELETE') }}
                          </form>
                          <a href="" onclick=" 
